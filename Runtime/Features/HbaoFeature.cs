...
namespace RoxamiRPCore
{
    ...
    {
        private const RenderPassEvent renderPassEvent = RenderPassEvent.AfterRenderingGbuffer;
        
        [SerializeField]
        private HBAoSettings settings = new ();
        
        private HBAoPass pass;
        private readonly BlurRenderPass blurPass = new ();

        public override void Create()
        {
            if (settings == null || !settings.computeShader) return;

            pass = new HBAoPass(settings, blurPass, renderPassEvent);
        }
        ...
        private class HBAoPass : ScriptableRenderPass
        {
            public HBAoPass(HBAoSettings settings, BlurRenderPass blurRenderPass, RenderPassEvent renderPassEvent)
            {
